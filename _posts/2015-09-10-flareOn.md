...
You'll see in my solutions that I had crocked logic, some luck and overall some hilarious code! Like the FLARE team say though, "There is no such thing as cheating in malware analysis".
...
We need the byte_402140 location to grab the values we are comparing each character of our password after the XOR. 
...
Opening the file in IDA I saw the main routine that asks for the string and traced it into the decryption loop at *sub_401084*. I noticed it was using a lot of weird x86 instructions and saw the location were it would validate a character in our inputted password. 
...
*loc_4010A2* causes the program to continue to traverse our password. Looking at the assembly closer I realized that eax is always xor'd when a character is not in the right location. I'll come back to this.
...
C:\Users\Topher\Downloads\flare-on\3-elfie - Copy>mv elfie elfie.py

C:\Users\Topher\Downloads\flare-on\3-elfie - Copy>elfie.py > whatNow.py
...
Getting the binary, *youPecks.exe* we can see it even tells us 2+2=4
...
Note in the code I had already found *flarebearstare* or had noticed it was being referenced in the subroutine in question. 
